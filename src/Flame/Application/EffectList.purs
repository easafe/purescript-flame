...
mount (QuerySelector selector) application = do
      maybeElement <- FAD.querySelector selector
      case maybeElement of
            Just parent -> run parent false application
...
run :: forall model message. DomNode -> Boolean -> Application model message -> Effect (Channel (Array message))
